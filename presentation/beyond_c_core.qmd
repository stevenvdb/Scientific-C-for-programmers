...
## External libraries overview {.smaller}
...
By loading a module on the cluster, some variables are set to make it easier to use libraries.
For instance `CPATH` will include the path of the header files, `LD_LIBRARY_PATH` will include the path of the libraries.
:::

## Minimizing a function with GSL

See [https://www.gnu.org/software/gsl/doc/html/min.html](https://www.gnu.org/software/gsl/doc/html/min.html) for documentation about one dimensional minimization
...
## Minimizing a function with GSL {.smaller}
...
## Handwriting recognition {.smaller}
...
::: {.fragment .callout-tip}
It is not necessary or even wanted to fully understand the mathematics of the model.
The following slides contain the essential things you need to know.
:::

## Handwriting recognition {.smaller background-color="lightgrey"}

### Supervised machine learning models:

::: {.incremental}
- Propose a model $y = f(x; A)$ that relates the input $x$ to the output $y$ 
  with a function $f$ that depends on (currently unknown) parameters $A$
- Collect a training set of known input-output pairs of $\{x, y\}$ and
  determine the parameters $A$ such that the model predicts the expected
  outputs for the training set
...
An excellent tutorial is available at [https://www.3blue1brown.com/lessons/neural-networks](https://www.3blue1brown.com/lessons/neural-networks)
(it uses multiple layers, so more parameters than in our case)

![](images/neural-network-function.png)

## Handwriting recognition {.smaller}

**Instructions:**

- Use [https://github.com/AndrewCarterUK/mnist-neural-network-plain-c](https://github.com/AndrewCarterUK/mnist-neural-network-plain-c) as a
  starting point
...
## Handwriting recognition discussion {.smaller}
...
## Segmentation fault example {.smaller}
...
## Versions of C ^[Adapted from https://github.com/gjbex/Scientific-C]
