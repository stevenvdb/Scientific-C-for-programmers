...
When turning on compiler optimizations, recursive functions might be automatically converted to a loop,
especially in case of tail recursion (where the recursive call is the last statement).
:::
...
## Monte Carlo $\pi$ timings {background-color="lightgrey"}
